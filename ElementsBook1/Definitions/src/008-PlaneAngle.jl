...
function plane_angle(center::Observable{Point2f}, pointA::Observable{Point2f},
    ...
    pl = [poly!(@lift([Point2f0(p) for p in vcat($(angle_data.angle_range), [$center])]),
                color=color, strokewidth=0f0),
          lines!(@lift([Point2f0($pointA), Point2f0($center)]),
                 color=linecolor, linewidth=(observable_width)),
          lines!(@lift([Point2f0($center), Point2f0($pointB)]),
